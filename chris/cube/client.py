...
class CubeClient(AuthenticatedClient[CubeCollectionLinks, 'CubeClient']):
    ...
    def get_compute_resources_of(self, plugin: CubePlugin) -> AsyncIterator[ComputeResource]:
    ...
    def search_compute_resources(self, max_requests=100, **query) -> AsyncIterator[ComputeResource]:
        return self.search(
            url=self.collection_links.compute_resources,
            query=query, element_type=ComputeResource,
